...
var MetaAgent = class {
   ...
   */
  async run({ inputs, history = [], historyFor, optionsFor }) {
    ...
    const insights = await Promise.all(
      agents.map(
        (a) => a.analyze(
          ...
          optionsFor ? optionsFor(a.domain.id) : void 0
...
var TemporalRunner = class {
   ...
   */
  async step(inputs, extraOptions) {
    const snapshot = this.previousSnapshot;
    const tickNow = this.currentTick;
    const temporal = {
      tick: tickNow,
      previousInput: (domainId) => snapshot?.inputs[domainId],
      previousInsight: (domainId) => snapshot?.insights[domainId]
    };
    const runInput = {
      inputs,
